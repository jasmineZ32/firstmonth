/**
 *	The game of Pig.
 *	(The user will play a game of pig aginst the computer, they have an option
 *   to roll a dice to gain points. They can roll or hold. Hold would keep all
 * 	 the dice numbers into teir game score, if they roll and get a 1, all those points
 * 	 will be lost. The computer will always hold at 20 turn points.)
 *
 *	@Jasmine	
 *	@September 13, 2024
 */

import java.util.Scanner;
public class PigGame {
	private Scanner kb;
	private Dice dice;
	
	PigGame()
	{
		kb = new Scanner(System.in);
	}
	
	public static void main(String [] args)
	{
		PigGame pg= new PigGame();
		pg.printIntroduction();
	}
	/**	Print the introduction to the game */
	public void printIntroduction() {
		System.out.println("\n");
		System.out.println("______ _         _____");
		System.out.println("| ___ (_)       |  __ \\");
		System.out.println("| |_/ /_  __ _  | |  \\/ __ _ _ __ ___   ___");
		System.out.println("|  __/| |/ _` | | | __ / _` | '_ ` _ \\ / _ \\");
		System.out.println("| |   | | (_| | | |_\\ \\ (_| | | | | | |  __/");
		System.out.println("\\_|   |_|\\__, |  \\____/\\__,_|_| |_| |_|\\___|");
		System.out.println("          __/ |");
		System.out.println("         |___/");
		System.out.println("\nThe Pig Game is human vs computer. Each takes a"
							+ " turn rolling a die and the first to score");
		System.out.println("100 points wins. A player can either ROLL or "
							+ "HOLD. A turn works this way:");
		System.out.println("\n\tROLL:\t2 through 6: add points to turn total, "
							+ "player's turn continues");
		System.out.println("\t\t1: player loses turn");
		System.out.println("\tHOLD:\tturn total is added to player's score, "
							+ "turn goes to other player");
		System.out.println("\n");
		System.out.print("Play game or Statistics (p or s) -> ");
		String letter = new String(" ");
		letter = kb.next();
		if(letter.equalsIgnoreCase("p"))
			playGame();
	}
	
	public void playGame()
	{
		int userScore = 0;
		int compScore = 0;
		int turnScore = 0;
		int diceNum = 0;
		boolean userTurn = true;
		String move = new String(" ");
		while(userScore <= 100 && compScore <= 100)
		{
			if (userTurn)
			{
				System.out.println("Your turn score: " + turnScore);
				System.out.println("Your total score; " + userScore);
				System.out.print("(r)oll or (h)old -> ");
				move = kb.next();
				System.out.println();
				if(move.equals("r"))
				{
					
				}
			}
		}
	}
	
}
